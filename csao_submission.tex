...
To ensure dummy data realism, our parsing pipeline preserved natural sparsity, varying cart sizes, and geographical imbalances inherent in the real-world Kaggle dataset, effectively simulating Zomato's messy production environment without injecting artificial leakage.
...
\subsection{Scenario A: Butter Chicken + Naan (Dinner, Premium)}

\textbf{Cart:} Butter Chicken Boneless + Butter Naan \hfill \textit{Latency: 380.06ms}
...
\midrule
1 & Malai Gud Pide & 0.888 & \Rs349 \\
2 & Bone in Peri Peri Grilled Chicken & 0.630 & \Rs387 \\
3 & Bone in Smoky Bbq Grilled Chicken & 0.617 & \Rs409 \\
4 & Chilli Cheese Garlic Bread & 0.543 & \Rs360 \\
5 & Bageecha Pizza & 0.360 & \Rs538 \\
\bottomrule
\end{tabular}
\end{center}
\textit{Analysis:} This represents a ``Cold-Start'' scenario where the graph has sparse edges for this specific combination. The system successfully falls back to BERT semantic embeddings, clustering the premium dinner items and gracefully filling the recommendation rail, maintaining a 100\% attach rate despite no exact historical match.

\subsection{Scenario B: Solo Rice Item (Dinner, Mid)}

\textbf{Cart:} Angara Rice \hfill \textit{Latency: 2.81ms}
...
\textbf{\#} & \textbf{Recommended Item} & \textbf{Score} & \textbf{Price} \\
\midrule
1 & Grilled Chicken Jamaican Tender & 0.963 & \Rs374 \\
2 & Bone in Jamaican Grilled Chicken & 0.792 & \Rs427 \\
\bottomrule
\end{tabular}
\end{center}
\textit{Analysis:} Extremely fast $O(1)$ graph lookup (2.81ms). The system correctly identifies that a customer ordering dry/flavored rice is highly likely to add a meat accompaniment (chicken tenders/grilled chicken). EV price boosting actively uplifts these high-margin proteins.
